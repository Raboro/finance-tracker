...
describe('PaymentItem', () => {
  test('correct payment value', () => {
    ...
    const { getByTestId } = render(<PaymentItem payment={payment} openModal={() => {}} />)
    expect(getByTestId('PaymentItem-'.concat(payment.getKey())).props.children).toBe(100);
  });

  test('correct value parsed to openModel', () => {
    const modal = jest.fn();
    const payment = new Payment(100, '0');
    const { getByTestId } = render(<PaymentItem payment={payment} openModal={modal} />)
