...
describe('Balance', () => {
  test('positive balance style', () => {
    expect(getStyling(true)).toMatchObject([{color: 'green'}, SHARED_STYLING])
  });

  test('zero balance style', () => {
    expect(getStyling(false, true)).toMatchObject([{color: 'black'}, SHARED_STYLING])
  });
...
function getRandomNumber(positive: boolean): number {
    let number = Math.random() * 99999 + 1
